...
public class RejectDonationServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DBConnection.getConnection()) {
            String sql = "UPDATE history_donations SET status='rejected' WHERE id=?";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, Integer.parseInt(donationId));
            int rows = ps.executeUpdate();

            if (rows > 0) {
                response.sendRedirect("adminDashboard.jsp?success=Donation+Rejected");
            } else {
